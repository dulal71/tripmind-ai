...
export default function RecommendationsPage() {
  ...
  const getMatchColor = (score: number) => {
    ...
    return 'text-zinc-500 dark:text-zinc-400 bg-zinc-500/15 border-zinc-500/30';
  };
    ...
    return (
      <div className="min-h-screen bg-white dark:bg-zinc-950 flex flex-col items-center justify-center px-4">
        <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/60 p-8 text-center max-w-md">
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-6">You need to be signed in to get AI recommendations.</p>
            ...
            <div className="sticky top-24 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/60 p-6">
              ...
              <div className="mb-6">
                ...
                {selectedInterests.length === 0 && (
                  <p className="text-xs text-zinc-400 dark:text-zinc-500 mt-2">Select at least one interest</p>
                )}
              </div>
            ...
            <AnimatePresence mode="wait">
              {getRecommendations.isPending ? (
                <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center justify-center py-20">
                  ...
                  <p className="mt-6 text-lg font-medium text-zinc-900 dark:text-white">AI is finding perfect destinations...</p>
                  <p className="mt-2 text-sm text-zinc-500 dark:text-zinc-400">Analyzing your preferences to find the best matches.</p>
                </motion.div>
              ) : recommendations ? (
                <motion.div key="results" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                  ...
                  {recommendations.map((rec, idx) => (
                    <motion.div
                      ...
                      className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/60 p-6 hover:border-zinc-300 dark:hover:border-zinc-700 transition-colors"
                    >
                      <div className="flex items-start justify-between mb-3">
                        <div>
                          <Link href={`/explore?search=${encodeURIComponent(rec.name)}`} className="text-xl font-bold text-zinc-900 dark:text-white hover:text-blue-500 dark:hover:text-blue-400 transition-colors">
                            {rec.name}
                          </Link>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {rec.bestFor.map((tag) => (
                          <span key={tag} className="rounded-full bg-zinc-100 dark:bg-zinc-800 px-2.5 py-0.5 text-xs font-medium text-zinc-600 dark:text-zinc-300">
                            {tag}
                          </span>
                        ))}
                      </div>
                ...
                <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center py-20">
                  <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/60 p-8 text-center max-w-md">
                    <FiTrendingUp className="mx-auto h-12 w-12 text-zinc-300 dark:text-zinc-600 mb-4" />
                    <h3 className="text-lg font-semibold text-zinc-700 dark:text-zinc-300">Discover Your Next Adventure</h3>
