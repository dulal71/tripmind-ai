    ...
    return (
      <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center px-4">
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-8 text-center max-w-md">
          ...
          <p className="text-sm text-zinc-400 mb-6">You need to be signed in to create a trip.</p>
          <Link
            href="/login"
            className="inline-flex items-center gap-2 rounded-lg bg-gradient-to-r from-blue-600 to-violet-600 px-6 py-2.5 text-sm font-semibold text-white"
          >
            Sign In
          </Link>
        </div>
    ...
    <div className="min-h-screen bg-zinc-950">
      <div className="relative overflow-hidden border-b border-zinc-800/60">
        ...
        <div className="relative mx-auto max-w-3xl px-4 pb-8 pt-12 sm:px-6">
          <Link href="/trips" className="inline-flex items-center gap-1.5 text-sm text-zinc-400 hover:text-white transition-colors mb-6">
          ...
          >
            ...
            <h1 className="text-3xl font-extrabold tracking-tight text-white">
              Create a <span className="bg-gradient-to-r from-blue-400 to-violet-400 bg-clip-text text-transparent">Trip</span>
            </h1>
            <p className="mt-3 text-base text-zinc-400">Plan your next adventure in a few steps.</p>
          </motion.div>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          {step === 2 && (
            <motion.div
              initial={{ opacity: 0, x: 20 }}
              animate={{ opacity: 1, x: 0 }}
              className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-6 space-y-6"
            >
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {STYLE_OPTIONS.map((opt) => (
                    <button
                      key={opt.value}
                      type="button"
                      onClick={() => setValue('travelStyle', opt.value)}
                      className={`rounded-xl border p-3 text-center text-sm font-medium capitalize transition-all ${
                        watch('travelStyle') === opt.value
                          ? 'border-blue-500/50 bg-blue-500/10 text-blue-400'
                          : 'border-zinc-800 bg-zinc-950/50 text-zinc-400 hover:border-zinc-700'
                      }`}
                    >
          ...
          <div className="mt-6 flex items-center justify-between">
            ...
            {step < totalSteps ? (
              ...
              >
                ...
                {!createTrip.isPending && <FiCheck className="h-4 w-4" />}
              </button>
            )}
          </div>
        </form>
