    ...
    return (
      <div className="min-h-screen bg-white dark:bg-zinc-950 flex flex-col items-center justify-center px-4">
        <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/60 p-8 text-center max-w-md">
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-6">You need to be signed in to use the AI Planner.</p>
      ...
      <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="lg:col-span-1">
            <div className="sticky top-24 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/60 p-6">
              <h2 className="text-lg font-semibold text-zinc-900 dark:text-white mb-6 flex items-center gap-2">
                <FiCompass className="h-5 w-5 text-blue-500 dark:text-blue-400" />
                Trip Details
              </h2>

              <div className="mb-5">
                <label className="block text-sm font-medium text-zinc-600 dark:text-zinc-300 mb-2">Destination</label>
                <div className="relative">
                  <select
                    id="planner-destination"
                    value={destinationId}
                    onChange={(e) => setDestinationId(e.target.value)}
                    className="w-full appearance-none rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-800/50 py-2.5 pl-3 pr-10 text-sm text-zinc-900 dark:text-white focus:border-blue-500/50 focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all"
                  >
                  ...
                  <FiChevronDown className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-zinc-400 dark:text-zinc-500" />
                </div>
                {!destinationsData && (
                  <p className="text-xs text-zinc-400 dark:text-zinc-500 mt-1.5">Loading destinations...</p>
                )}
              </div>
              ...
              <div className="mb-5">
                <label className="block text-sm font-medium text-zinc-600 dark:text-zinc-300 mb-2">Budget Level</label>
                    ...
                    <button
                      ...
                      className={`rounded-lg px-3 py-2 text-sm font-medium transition-all cursor-pointer ${
                        budget === opt.value
                          ...
                          : 'border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-800/50 text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white hover:border-zinc-300 dark:hover:border-zinc-700'
                      }`}
                    >
                      {opt.label}
                    </button>
              ...
              <div className="mb-5">
                <label className="block text-sm font-medium text-zinc-600 dark:text-zinc-300 mb-2">Travel Style</label>
                <div className="grid grid-cols-2 gap-2">
              ...
              <div className="mb-5">
                <label className="block text-sm font-medium text-zinc-600 dark:text-zinc-300 mb-2">Travelers</label>
                <div className="flex items-center gap-3">
                  <button
                    onClick={() => setTravelerCount(Math.max(1, travelerCount - 1))}
                    className="flex h-9 w-9 items-center justify-center rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-800/50 text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white hover:border-zinc-300 dark:hover:border-zinc-700 transition-colors cursor-pointer"
                  >
                    -
                  </button>
                  <span className="text-lg font-semibold text-zinc-900 dark:text-white w-8 text-center">{travelerCount}</span>
                  <button
              ...
              <div className="mb-6">
                <label className="block text-sm font-medium text-zinc-600 dark:text-zinc-300 mb-2">Interests</label>
            ...
            <AnimatePresence mode="wait">
              {generateItinerary.isPending ? (
                <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center justify-center py-20">
                  ...
                  <p className="mt-6 text-lg font-medium text-zinc-900 dark:text-white">AI is crafting your itinerary...</p>
                  <p className="mt-2 text-sm text-zinc-500 dark:text-zinc-400">This may take a moment. Creating a personalized plan just for you.</p>
                </motion.div>
              ) : itinerary ? (
                <motion.div key="results" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                  <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/60 p-6">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      {itinerary.budgetAnalysis.breakdown.map((item) => (
                        <div key={item.category} className="rounded-lg bg-zinc-100 dark:bg-zinc-800/50 p-3">
                          <p className="text-xs text-zinc-400 dark:text-zinc-500 uppercase tracking-wider">{item.category}</p>
                          ...
                          <div className="mt-2 h-1.5 rounded-full bg-zinc-200 dark:bg-zinc-700">
                            <div className="h-full rounded-full bg-gradient-to-r from-blue-500 to-violet-500" style={{ width: `${item.percentage}%` }} />
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  {itinerary.days.map((day) => (
                    <div key={day.dayNumber} className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/60 p-6">
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        <div>
                          <h3 className="text-lg font-semibold text-zinc-900 dark:text-white">Day {day.dayNumber}</h3>
                          <p className="text-sm text-zinc-500 dark:text-zinc-400">{day.theme}</p>
                        </div>
                      </div>
                      <div className="space-y-3">
                        {day.activities.map((activity, idx) => (
                          <div key={idx} className="flex gap-3 rounded-lg bg-zinc-100 dark:bg-zinc-800/30 p-3">
                            <div className="flex items-center gap-1.5 text-xs text-zinc-400 dark:text-zinc-500 whitespace-nowrap min-w-[70px]">
                    ...
                    <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/60 p-6">
                      ...
                      <ul className="space-y-2">
                        {itinerary.aiTips.map((tip, idx) => (
                          <li key={idx} className="text-sm text-zinc-600 dark:text-zinc-300 leading-relaxed">
                            {tip}
                          </li>
                        ))}
                      </ul>
                ...
                <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center py-20">
                  <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/60 p-8 text-center max-w-md">
                    <FiStar className="mx-auto h-12 w-12 text-zinc-300 dark:text-zinc-600 mb-4" />
                    <h3 className="text-lg font-semibold text-zinc-700 dark:text-zinc-300">Your AI Itinerary</h3>
