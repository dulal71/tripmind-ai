...
export default function ChatPage() {
    ...
    return (
      <div className="min-h-screen bg-white dark:bg-zinc-950 flex flex-col items-center justify-center px-4">
        <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/60 p-8 text-center max-w-md">
          <FiMessageCircle className="mx-auto h-12 w-12 text-zinc-300 dark:text-zinc-600 mb-4" />
          <h2 className="text-xl font-semibold text-zinc-900 dark:text-white mb-2">Sign in required</h2>
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-6">You need to be signed in to use the AI Chat Assistant.</p>
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-zinc-950 flex flex-col">
      {/* Header */}
      <div className="border-b border-zinc-200 dark:border-zinc-800/60 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-xl">
        <div className="mx-auto max-w-4xl px-4 py-4 sm:px-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-zinc-900 dark:text-white">AI Travel Assistant</h1>
              <p className="text-xs text-zinc-500 dark:text-zinc-400">Ask me anything about travel</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={startNewChat}
              className="flex items-center gap-1.5 rounded-lg border border-zinc-200 dark:border-zinc-800 px-3 py-2 text-sm text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white hover:bg-zinc-100 dark:hover:bg-zinc-800/50 transition-colors cursor-pointer"
            >
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Sidebar */}
        <div className="hidden lg:flex w-64 border-r border-zinc-200 dark:border-zinc-800/60 bg-zinc-50 dark:bg-zinc-950/80 flex-col">
          <div className="p-4 border-b border-zinc-200 dark:border-zinc-800/60">
            <h2 className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">Recent Chats</h2>
          </div>
          <div className="flex-1 overflow-y-auto p-2 space-y-1">
            {loadingHistory ? (
              <div className="flex items-center justify-center py-8">
                <FiLoader className="h-5 w-5 text-zinc-400 dark:text-zinc-500 animate-spin" />
              </div>
            ) : chatHistories.length === 0 ? (
              <p className="text-xs text-zinc-400 dark:text-zinc-500 text-center py-8">No chat history yet</p>
            ) : (
              chatHistories.map((h) => (
                <button
                  key={h._id}
                  ...
                  className={`w-full text-left rounded-lg p-3 transition-colors group cursor-pointer ${
                    historyId === h._id ? 'bg-zinc-200 dark:bg-zinc-800/60 text-zinc-900 dark:text-white' : 'text-zinc-500 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800/40 hover:text-zinc-700 dark:hover:text-zinc-200'
                  }`}
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 p-1 text-zinc-400 dark:text-zinc-500 hover:text-red-400 transition-all cursor-pointer"
                    >
          ...
          <div className="mx-auto max-w-4xl px-4 py-6 sm:px-6">
            {messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16">
                <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/60 p-8 text-center max-w-lg w-full">
                  <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-blue-500/20 to-violet-500/20">
                    <FiMessageCircle className="h-8 w-8 text-blue-500 dark:text-blue-400" />
                  </div>
                  <h2 className="text-xl font-semibold text-zinc-900 dark:text-white mb-2">How can I help you?</h2>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {SUGGESTED_PROMPTS.map((prompt) => (
                      <button
                        ...
                        className="rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-800/50 p-3 text-left text-sm text-zinc-600 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-white hover:border-zinc-300 dark:hover:border-zinc-700 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-all cursor-pointer"
                      >
                        {prompt}
                      </button>
                    ))}
              ...
              <div className="space-y-6">
                      ...
                      <div
                        className={`max-w-[80%] rounded-2xl px-4 py-3 ${
                          msg.role === 'user'
                            ...
                            : 'border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/60 text-zinc-700 dark:text-zinc-300'
                        }`}
                      >
                        <p className="text-sm whitespace-pre-wrap leading-relaxed">{msg.content}</p>
                      </div>
                      {msg.role === 'user' && (
                        <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-zinc-200 dark:bg-zinc-800 shrink-0 mt-1">
                ...
                {chatMutation.isPending && (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex gap-3">
                    ...
                    <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/60 px-4 py-3">
                      <div className="flex items-center gap-2">
                        <FiLoader className="h-4 w-4 text-blue-500 dark:text-blue-400 animate-spin" />
      ...
      {/* Input Area */}
      <div className="border-t border-zinc-200 dark:border-zinc-800/60 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-xl">
